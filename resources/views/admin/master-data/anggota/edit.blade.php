...
@section('subtitle', 'Perbarui informasi anggota koperasi')
...
@section('content')
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ route('admin.master-data.anggota.update', $member) }}" class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    <label class="text-sm font-semibold text-slate-700">Nomor Anggota</label>
                    <p class="mt-2 text-sm font-semibold text-slate-900">{{ $member->member_no }}</p>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700" for="name">Nama Anggota <span class="text-rose-500">*</span></label>
                    <input id="name" name="name" type="text" value="{{ old('name', $member->name) }}" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0" />
                </div>
                <div>
                    ...
                    <select id="member_type" name="member_type" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0">
                        @foreach (['SISWA' => 'Siswa', 'GURU' => 'Guru', 'KARYAWAN' => 'Karyawan'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('member_type', $member->member_type) === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div id="class-field" class="hidden">
                    ...
                    <input id="class_name" name="class_name" type="text" value="{{ old('class_name', $member->class_name) }}" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0" />
                ...
                <div>
                    ...
                    <input id="phone" name="phone" type="text" value="{{ old('phone', $member->phone) }}" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0" />
                </div>
                <div class="lg:col-span-2">
                    <label class="text-sm font-semibold text-slate-700" for="address">Alamat</label>
                    <textarea id="address" name="address" rows="3" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0">{{ old('address', $member->address) }}</textarea>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700" for="join_date">Tanggal Bergabung <span class="text-rose-500">*</span></label>
                    <input id="join_date" name="join_date" type="date" value="{{ old('join_date', $member->join_date?->format('Y-m-d')) }}" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0" />
                </div>
                <div>
                    ...
                    <select id="status" name="status" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none focus:ring-0">
                        ...
                        <option value="KELUAR" @selected(old('status', $member->status) === 'KELUAR')>Keluar</option>
