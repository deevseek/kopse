...
@section('subtitle', 'Formulir penambahan jenis simpanan')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.master.jenis-simpanan.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="is_withdrawable" name="is_withdrawable" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none">
                            <option value="1" @selected(old('is_withdrawable') === '1')>Bisa</option>
                            <option value="0" @selected(old('is_withdrawable', '0') === '0')>Tidak</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select id="is_periodic" name="is_periodic" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none">
                            <option value="1" @selected(old('is_periodic') === '1')>Ya</option>
                            <option value="0" @selected(old('is_periodic', '0') === '0')>Tidak</option>
                        </select>
                        @error('is_periodic')
                            <p class="mt-2 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="default_amount" name="default_amount" type="number" step="0.01" value="{{ old('default_amount') }}" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none" />
