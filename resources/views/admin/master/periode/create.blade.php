...
@section('subtitle', 'Formulir penambahan periode buku')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.master.periode.store') }}" class="space-y-6">
                @csrf

                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        <label class="text-sm font-medium text-slate-600" for="year">Tahun Buku</label>
                        <input id="year" name="year" type="text" value="{{ old('year') }}" placeholder="2024/2025" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none" />
                        @error('year')
                    ...
                    <div>
                        ...
                        <input id="start_date" name="start_date" type="date" value="{{ old('start_date') }}" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none" />
                        @error('start_date')
                            <p class="mt-2 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input id="end_date" name="end_date" type="date" value="{{ old('end_date') }}" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none" />
                        @error('end_date')
                            <p class="mt-2 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select id="is_active" name="is_active" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none">
                            ...
                            <option value="0" @selected(old('is_active', '0') === '0')>Nonaktif</option>
                        </select>
                        @error('is_active')
                            <p class="mt-2 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="is_closed" name="is_closed" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:border-slate-400 focus:outline-none">
                            <option value="0" @selected(old('is_closed', '0') === '0')>Terbuka</option>
                            <option value="1" @selected(old('is_closed') === '1')>Ditutup</option>
                        </select>
                        @error('is_closed')
                            <p class="mt-2 text-xs text-rose-500">{{ $message }}</p>
                        @enderror
